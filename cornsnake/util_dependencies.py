...
def _raise_versions_error(program: str, message: str) -> None:
    ...
    """
    raise SystemExit(
        f"DEPENDENCIES ERROR: {message}. Please check your version of {program}"
    )
...
def _check_major_versions_equal(program: str, actual: int, expected: int) -> None:
    ...
    """
    if actual != expected:
        _raise_versions_error(
            program,
            f"{program} major version must be {expected} - but you are using {actual}",
        )
...
def _check_minor_version_greater_than_or_equal(
    ...
    if actual < expected_minor:
        _raise_versions_error(
            program,
            f"{program} version must be at least {expected_major}.{expected_minor} - but you are using {expected_major}.{actual}",
        )
...
def check_python_version() -> None:
    ...
    _check_minor_version_greater_than_or_equal(
        program, version.minor, PY_MAJOR, PY_MIN_MINOR
    )
...
def check_git_version() -> None:
    ...
    _check_minor_version_greater_than_or_equal(
        program, actual_minor, GIT_MAJOR, GIT_MIN_MINOR
    )
