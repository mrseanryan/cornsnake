...
def _parse_result(result: str) -> list[str]:
    """
...
def get_current_branch(local_repo_location: str) -> str:
    ...
    return execute_command(
        "rev-parse", ["--abbrev-ref", "HEAD"], local_repo_location
    ).strip()
...
def get_last_commit_id(path_to_repo_dir: str, file_only: str | None = None) -> str:
    ...
    git_args = [
        ...
        "-n",
        "1",
        f'--pretty=format:"%H{COMMIT_PARTS_SEPARTOR}%ad{COMMIT_PARTS_SEPARTOR}%s"',
        "--date=short",
    ]
    if file_only is not None:
        git_args += [file_only]
    result = util_proc.run_process_and_get_output(
        config.PATH_TO_GIT, git_args, path_to_repo_dir
    )
...
def is_git_repo(repo_dir: str) -> bool:
    ...
    return os.path.exists(path_to_git_config)
...
def get_git_remote_push_url(repo_dir: str) -> str:
    ...
    try:
        ...
        return execute_command(
            "remote", ["get-url", "--push", "origin"], repo_dir
        ).rstrip()
...
def execute_command(command: str, git_args: list[str], working_dir: str) -> str:
    ...
    return util_proc.run_process_and_get_output(
        config.PATH_TO_GIT, git_args_with_command, working_dir
    )
...
def fetch_notes(working_dir: str) -> None:
    ...
    working_dir (str): The working directory of the repository."""
    execute_command(
        "fetch", ["origin", "refs/notes/*:refs/notes/*", "--force"], working_dir
    )
...
def checkout_at_start_of_date(
    ...
    last_commit = execute_command(
        "rev-list",
        ["-n", "1", "--first-parent", f'--before="{start_date} 00:00"', branch],
        local_repo_location,
    )
    last_commit = last_commit.strip()
    if last_commit:
        checkout_branch(last_commit, local_repo_location)
        return True
    return False
...
def get_commits_after_date(
    ...
    for branch in branches:
        ...
        result = execute_command(
            "log", ['--pretty=format:"%H"', f'--after="{day_before_cutoff}"'], repo_dir
        )
    ...
    try:
        if '"' in path_to_text_editor:
            if path_to_text_editor.count('"') == 2:
                parts = path_to_text_editor.split(
                    '"'
                )  # '"path to exe" cmd' -> ['', 'path to exe', ' cmd']
...
def delete_all_tags(path_to_local_repo: str) -> None:
    ...
    execute_command_in_chunks(
        "push", ["--delete", "origin", "--quiet"], tags, path_to_local_repo
    )
...
def get_config(key: str, path_to_local_repo: str) -> str:
    ...
    try:
        ...
        if len(re.args) == 2 and re.args[1] == 1:
            if config.IS_VERBOSE:
                util_print.print_custom(
                    f"[ok] git config has no value for setting '{key}'"
                )
            return ""
        raise
    return result
...
def is_git_ignored(directory: str, filename: str) -> bool:
    ...
    try:
        execute_command("check-ignore", [filename], directory)
    except RuntimeError as re:
        if len(re.args) == 2:
            exit_code = re.args[1]
            if exit_code == 0:
